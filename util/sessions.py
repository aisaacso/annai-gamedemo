...
class Session(object):
    ...
    def __init__(self):
        ...
        if self.cookie.get(COOKIE_NAME):
            ...
            try:
                self.session = memcache.get(self.key) #@UndefinedVariable
            except:
                self.session = None
        ...
        if self.session is None:
            ...
            self.session = dict()
            memcache.add(self.key, self.session, 3600) #@UndefinedVariable
    ...
    def _update_cache(self):
        memcache.replace(self.key, self.session, 3600) #@UndefinedVariable
